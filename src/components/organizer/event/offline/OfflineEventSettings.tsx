...
export interface IOfflineBookingSettingsData {
...
const OfflineEventSettings = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  const [formData, setFormData] = useState<IOfflineBookingSettingsData>({
  ...
  const handleSwitchChange = (name: keyof IOfflineBookingSettingsData) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(
        saveBookingSettings({ id: id as string, settings: formData })
      ).unwrap();
      ...
      <div className="p-4 md:p-6 bg-white rounded-lg shadow">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <span className="block text-sm font-medium text-gray-700">
                I do not wish to offer my customers with option to cancel their
...
export default OfflineEventSettings;
